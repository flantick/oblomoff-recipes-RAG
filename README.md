...
| `avg_steps`, `with_amounts` | детализация ответа и доля ингредиентов с граммовками (потолок задан корпусом — см. оговорки ниже) |
...
- `with_amounts` = 0.155 упирается в сам корпус, а не в извлечение. Обломов
  готовит на глаз: на 651 ролик приходится 2206 упоминаний вида «число +
  единица» — 3.4 на ролик, — а расплывчатых мер втрое больше («чуть-чуть»
  2061, «примерно» 1498, «где-то» 916, «по вкусу» 410). Промпт запрещает
  дописывать то, чего нет в тексте, поэтому низкое значение здесь —
  правильное поведение: альтернатива не «рецепт с граммовками», а рецепт с
  выдуманными граммовками. Метрику оставляем сторожем регрессии в обратную
  сторону: заметный рост означал бы, что модель начала сочинять количества.
