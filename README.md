...
удобнее гонять на GPU с хоста, погасив vLLM — **58 с на весь набор вместо 21
минуты**. Контейнер `api` собран с CPU-колесом torch и для этого не годится:

```bash
docker compose stop vllm
EMBED_DEVICE=cuda RERANK_DEVICE=cuda python -m src.eval.run --mode retrieval
docker compose start vllm
```

Ранжирующие метрики (`hit@k`, `MRR`) при этом совпадают с прод-прогоном до
четвёртого знака, а вот **скоры — нет**: `neg_score_max` 0.231 на CPU против
0.358 на GPU. Поэтому пороги (`RETRIEVAL_MIN_SCORE`) калибруются только в
прод-среде — в контейнере и на той же конфигурации `retrieve()`, что в бою:
у широкого прогона другой пул кандидатов и, соответственно, другие скоры.
...
- **терпимый разбор `source_n`** — модель регулярно возвращает `[3, 4]` или
  `"[1]"` вместо `3`, схема падала, и полностью извлечённый рецепт
  выбрасывался с `found=false`. Снаружи это выглядело как «в расшифровках нет
  ответа» на запрос вроде «борщ», который система на самом деле знает;
...
- **порог релевантности перед генерацией** — мусорные запросы («стекло,
  гвозди, бумага») отсекаются до вызова LLM, экономя ~25 с.

Галлюцинаций в этом прогоне нет: все восемь негативных запросов вернули
`found=false`. Оговорка: `found_acc` не детерминирована — при `temperature=0.2`
запрос «рецепт счастья» в отдельных прогонах всё же получает ответ (система
находит ролик про чили). Классы по скору реранкера пересекаются, поэтому
порогом это не лечится — см. комментарий у `RETRIEVAL_MIN_SCORE`.
