...
| Retrieval (34 позитивных запроса) | | Генерация (все 38) | |
|---|---|---|---|
| MRR | 0.863 | found_acc | 0.868 |
| hit@1 | 0.794 | attribution | 0.931 |
| hit@3 / hit@5 | 0.941 | avg_steps | 11.0 |
| ctx_precision | 0.727 | avg_ingredients | 9.3 |
| edge_rate | 0.120 | with_amounts | 0.147 |

По типам запроса: `exact` hit@1 = 0.88, `paraphrase` = 0.83,
**`descriptive` = 0.50** — запросы без названия блюда просаживаются вдвое.

Галлюцинаций нет: на всех четырёх негативных запросах система вернула
`found=false`. Все расхождения по `found_acc` — ложные отказы, то есть ошибка
уходит в сторону «не ответил», а не «выдумал».

Известные ограничения этого замера: набор мал (34 позитивных запроса — один
переехавший запрос двигает `hit@1` на 3 п.п.), и часть запросов размечена
неполно — у широких формулировок в `relevant` перечислены не все подходящие
ролики, поэтому реальные `hit@k` не ниже приведённых.
